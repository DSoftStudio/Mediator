...
- **Send(object) / Publish(object) benchmarks** — New isolated benchmark classes for all 4 libraries measuring runtime-typed dispatch overhead vs generic dispatch.
...
- **Packaging regression tests** — New `BuildTransitivePropsTests` and `InterceptorNamespaceCompilationTests` ensuring both `build/` and `buildTransitive/` props files stay in sync and guarding against CS9137 regressions via Roslyn in-memory compilation.
...
- **Shared `InterceptorHelpers` dispatch body builder** — `AppendSendDispatchBody` extracted to a single static method shared by `SendInterceptorGenerator` and `MediatorExtensionsGenerator`. Ensures dispatch logic is defined once, with `isRelease` parameter for branchless castclass vs isinst + virtual-dispatch fallback.
- **Typed extensions use defensive dispatch** — `MediatorExtensionsGenerator` now emits `isinst` + virtual-dispatch fallback (never branchless castclass), ensuring typed extensions work correctly in test projects and mocking scenarios.
- **Benchmark suite restructured** — Deleted legacy combined benchmark classes and `run-all-benchmarks.cmd`. Each library now has isolated per-category benchmark classes under `Benchmarks/{Library}/` directories.
