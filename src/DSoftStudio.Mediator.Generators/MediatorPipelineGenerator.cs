...
public sealed class MediatorPipelineGenerator : IIncrementalGenerator
{
        ...
        System.Collections.Immutable.ImmutableArray<BehaviorRegistration> behaviorRegistrations)
    {
        ...
        sb.AppendLine("                    // chain DEPENDENCY and constrains it exactly as the components and the handler do.");
        sb.AppendLine("                    // Without this a Scoped observer alongside all-singleton components yields a singleton");
        sb.AppendLine("                    // chain that captures the observer and its scoped graph -- ValidateScopes throws on the");
        sb.AppendLine("                    // first dispatch, and with validation off it silently shares them for the process lifetime.");
        sb.AppendLine("                    if (descriptor.Lifetime != global::Microsoft.Extensions.DependencyInjection.ServiceLifetime.Singleton)");
        sb.AppendLine("                        allSingleton = false;");
        sb.AppendLine();
        sb.AppendLine("                    // And a TRANSIENT observer takes the chain all the way down, for the same reason a");
        sb.AppendLine("                    // Transient component does: a Scoped chain is CACHEABLE, so it would construct the");
        sb.AppendLine("                    // observer once for the scope and hand every dispatch the same instance -- an explicit");
        sb.AppendLine("                    // Transient silently honoured as per-scope. The OR over descriptors is right here:");
        sb.AppendLine("                    // observers are resolved as an ENUMERABLE, so every registered one runs.");
        sb.AppendLine("                    if (descriptor.Lifetime == global::Microsoft.Extensions.DependencyInjection.ServiceLifetime.Transient)");
        sb.AppendLine("                        hasTransientChainDependency = true;");
        sb.AppendLine("                }");
