...
public sealed class StreamGenerator : IIncrementalGenerator
{
        ...
        EquatableArray<BehaviorTypeInfo> behaviors)
    {
        ...
        sb.AppendLine("                }");
        sb.AppendLine("                else if (descriptor.ServiceType == typeof(global::DSoftStudio.Mediator.Abstractions.IStreamRequestHandler<TRequest, TResponse>))");
        sb.AppendLine("                {");
        sb.AppendLine("                    // StreamPipelineChainHandler's ctor consumes the stream handler, so its lifetime");
        sb.AppendLine("                    // constrains the chain. RECORDED rather than folded here: this service type resolves");
        sb.AppendLine("                    // SINGLE, so the LAST descriptor is the one the container will hand the chain, and");
        sb.AppendLine("                    // reading the others would let a superseded registration decide. The behaviors above");
        sb.AppendLine("                    // keep the OR because they are ENUMERABLE and every descriptor for them is live.");
        sb.AppendLine("                    // It does NOT set hasBehaviors: a handler on its own never needs a chain.");
        sb.AppendLine("                    handlerLifetime = descriptor.Lifetime;");
        sb.AppendLine("                }");
        sb.AppendLine("            }");
        sb.AppendLine();
        sb.AppendLine("            // The stream handler, folded once the winning descriptor is known. A Transient one takes the");
        sb.AppendLine("            // chain all the way down for the same reason the request side does it: a Scoped chain is");
        sb.AppendLine("            // cacheable, so it would construct the handler once for the scope and share it -- and the");
        sb.AppendLine("            // transient dependency that kept it Transient -- across every enumeration in that scope.");
